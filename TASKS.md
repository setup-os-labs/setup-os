...
| SO-103 | done | Codex | `codex/extraction-observability-traceability` | Add extraction observability and traceability review | Desktop and generated files show processed inputs, draft counts, rejected noise, low-confidence items, conflicts, source checksums, and evidence locations |
...
| SO-115 | done | Codex | `codex/kt005-retest-navigation-runtime` | Fix KT-005 retest navigation and repo-root diagnostics | Primary sidebar items render visibly distinct first-screen app views instead of changing content below onboarding, installed runtime diagnostics explain the `SETUP_OS_REPO_DIR` workaround, and KT-005 can separate UI navigation from the remaining bundled-engine gap |
| SO-116 | done | Codex | `codex/packaged-engine-resource-resolver` | Bundle packaged Setup OS engine resources | Desktop release artifacts include `setup_os` and `examples` resources, the installed app resolves that packaged engine without `SETUP_OS_REPO_DIR`, and release checks guard the resource contract |
| SO-117 | done | Codex | `codex/local-observability-no-console` | Add local-first desktop observability and hide subprocess consoles | Windows desktop subprocesses run without flashing terminal windows, desktop actions write readable local logs, and ADR 0008 defines optional OSS observability adapters |
| SO-118 | todo | Codex | `codex/public-launch-clean-repo` | Create fresh public repo from final cleaned tree at public launch | Public launch repo starts from the final scrubbed source tree with no private workshop PRs, historical commits, branch names, or personal planning traces |
