...
Portfolio snapshots, transactions, cash balances, and watchlists are local CSV imports only; no broker credentials are stored.
...
def main() -> int:
    ...
    holdings_value = sum(float(row["quantity"]) * float(row["price"]) for row in holdings)
